// logica core de teles y garages
...
LoadTeles()
{
	...
	new Cache:cacheid, exist, count, gcount;
    for (new i=0; i!=MAX_TELES_COUNT; i++)
	{
		cacheid = FetchTeleDB(i);
...
LoadGarageEx(teleid, g)
{
	...
	new loaded;
	if(teleid < teleidGo){
		new Cache:cacheid, garageExist;

		cacheid = FetchGarageExDB(g);
		...
		if(!garageExist){
			...
			cache_get_value_name_int(0, "Creado", GaragesEx[g][Creado]);
...
CrearGarage(playerid, teleID){
	...
	if(garageID != -1){
		...
		{
			if(Teles[teleID][Interior] != 0 || Teles[teleID][World] != 0)
			{
				if(Teles[Teles[teleID][PickupIDGo]][Interior] != 0 && Teles[Teles[teleID][PickupIDGo]][World] != 0)
				{
					SendInfoMessage(playerid, 0, "", "Este tele no lleva a ningun exterior!");
					return false;
				}
			}
				...
				myInt == Teles[teleID][Interior])
			{
				...
				format(string, sizeof(string), "Estableciendo la posicion \"Uno\" del garage para el tele %i vinculado al tele %i", teleID, Teles[teleID][PickupIDGo]);
				SendInfoMessage(playerid, 0, "", string);
				SendInfoMessage(playerid, 0, "", "Use \"/Crear Garage\" nuevamente para establecer la posicion \"Dos\"");
			}
			else {
				SendInfoMessage(playerid, 0, "", "Debe acercarse al tele para crear un garage!");
			}
		}
...
ShowEditTeleNombre(playerid, teleid)
{
	...
	format(caption, sizeof(caption), "{"COLOR_AZUL"}Tele %i - {"COLOR_VERDE"}%i", teleidGo, teleid);
	strcat(caption, " -> Editar Nombre");

	format(info, sizeof(info), "{"COLOR_CREMA"}Nombre actual: %s\nIngrese el nuevo nombre:", Teles[teleid][LugarText]);
...
ShowTeleUbicaciones(playerid, teleid)
{
	...
	format(caption, sizeof(caption), "{"COLOR_AZUL"}Tele %i - {"COLOR_VERDE"}%i", teleidGo, teleid);
	strcat(caption, " -> Ubicaciones");

	format(info, sizeof(info), "{"COLOR_CREMA"}Ajustes\tOpciones\n");
	if(Teles[teleid][IsGarage])
	format(info, sizeof(info), "%sGarage\t{"COLOR_VERDE"}Ver/Editar\n", info);
	else
	format(info, sizeof(info), "%sSin garage\t\n", info);
	...
	format(info, sizeof(info), "%sMi Ubicacion\t{"COLOR_VERDE"}Mover Aqui\n", info);
	ShowPlayerDialogEx(playerid, 177, DIALOG_STYLE_TABLIST_HEADERS, caption, info, "Editar", "Volver");
}
